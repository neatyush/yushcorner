  ...
  return (
    <section className="mt-10 px-4 lg:px-40 m-10">
      ...
      <h1 className="font-nanito text-3xl lg:text-5xl font-bold text-center text-[#004e98] mb-4">
        {FoodWinePairingData.title}
      </h1>
      <p className="text-center text-sm text-gray-500 mb-6">
        Published on {FoodWinePairingData.date}
      </p>

       {/* Banner Image */}
            <div className="rounded-2xl overflow-hidden shadow-md mb-8 mt-10">
              <img
                src={FoodWinePairingData.bannerImg}
                alt="Blog Banner"
      ...
      <p className="text-[18px] font-ubuntu text-gray-800 leading-relaxed mb-6">
        {FoodWinePairingData.intro}
      </p>

      {/* Sections */}
      <div className="space-y-6">
        {FoodWinePairingData.sections.map((sec, idx) => (
          <div
            key={idx}
            className="p-4 rounded-xl bg-[#f9fafc] border border-gray-200 shadow-sm"
          >
            <h2 className="text-xl text-[#004e98] font-semibold mb-2">{sec.heading}</h2>
            <p className="mt-2 text-gray-700 text-[16px] font-ubuntu whitespace-pre-line">
              {sec.note}
            </p>
          </div>
        ))}
...
export default FoodWinePairing;
